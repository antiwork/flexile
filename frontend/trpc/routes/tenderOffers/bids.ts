...
export const tenderOffersBidsRouter = createRouter({
  list: companyProcedure
    ...
    .query(async ({ ctx, input }) => {
      if (!ctx.companyAdministrator && (!ctx.companyInvestor || ctx.companyInvestor.externalId !== input.investorId))
  ...
  create: companyProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      if (!ctx.companyInvestor) throw new TRPCError({ code: "FORBIDDEN" });
