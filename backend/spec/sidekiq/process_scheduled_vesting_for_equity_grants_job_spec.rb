...
RSpec.describe ProcessScheduledVestingForEquityGrantsJob do
  describe "#perform" do
    ...
    let!(:equity_grant_vests_as_per_schedule2) { create(:equity_grant, :vests_as_per_schedule, vesting_schedule:, period_ended_at: 1.day.from_now) }
    ...
    it "enqueues ProcessEquityGrantScheduledVestingJob for scheduled equity grants that have not ended" do
      ...
      expect(ProcessEquityGrantScheduledVestingJob).to have_enqueued_sidekiq_job(equity_grant_vests_as_per_schedule2.id)
