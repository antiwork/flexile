...
class InvoiceEquityCalculator
  ...
  def calculate
    ...
    equity_percentage = company.equity_enabled? ? company_worker.equity_percentage : 0
    equity_amount_in_cents = ((service_amount_cents * equity_percentage) / 100.to_d).round
    equity_amount_in_options = share_price_usd.present? ? (equity_amount_in_cents / (share_price_usd * 100.to_d)).round : nil
