...
class DividendRoundPresenter
  ...
  def summary
    {
      id: @dividend_round.external_id,
      total_amount_in_cents: @dividend_round.total_amount_in_cents,
      total_amount_in_usd: @dividend_round.total_amount_in_cents / 100.0,
      issued_at: @dividend_round.issued_at.iso8601,
      ...
      created_at: @dividend_round.created_at.iso8601,
      updated_at: @dividend_round.updated_at.iso8601,
  ...
  private
      ...
      @dividend_round.dividends.includes(company_investor: :user).map do |dividend|
        {
          ...
          total_amount_in_usd: dividend.total_amount_in_cents / 100.0,
          number_of_shares: dividend.number_of_shares,
          qualified_dividend_amount_in_cents: dividend.qualified_dividend_amount_in_cents,
          qualified_dividend_amount_in_usd: dividend.qualified_dividend_amount_in_cents / 100.0,
          non_qualified_dividend_amount_in_usd: (dividend.total_amount_in_cents - dividend.qualified_dividend_amount_in_cents) / 100.0,
    ...
    def calculate_payment_fees
      ...
      
      {
        ...
        dividend_amount_usd: dividend_amount / 100.0,
        processing_fee_cents: processing_fee,
        processing_fee_usd: processing_fee / 100.0,
        transfer_fee_cents: transfer_fee,
        transfer_fee_usd: transfer_fee / 100.0,
        total_with_fees_cents: dividend_amount + processing_fee + transfer_fee,
        total_with_fees_usd: (dividend_amount + processing_fee + transfer_fee) / 100.0
      }
    end
end